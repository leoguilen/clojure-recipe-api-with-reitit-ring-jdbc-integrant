...
(defn create-recipe!
  [db]
  (fn [request]
    (let [recipe-id (str (java.util.UUID/randomUUID))
...
(defn unfavorite-recipe!
  ...
  (fn [request]
    (let [uid (-> request :claims :sub)
          recipe-id (-> request :parameters :path :recipe-id)
          unfavorited? (recipe-db/unfavorite-recipe! db {:uid uid :recipe-id recipe-id})] 
      (if unfavorited?
        (rr/status 204) 
        (rr/not-found {:type "recipe-not-found"
                    :message "Recipe not found"
...
(defn create-ingredient!
  ...
  (fn [request]
    (let [recipe-id (-> request :parameters :path :recipe-id)
          ...
          ingredient-id (str (java.util.UUID/randomUUID))]
